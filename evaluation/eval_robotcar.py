...
from transforms import CenterCrop,ResizeWithIntrinsic
...
def evaluate():
    ...
    for i in trange(gt_len):
        ...
        mask = (gt > args.min_depth) & (gt < args.max_depth)   # 保留的
        gt_h, gt_w = gt.shape
        pred = cv2.resize(pred, (gt_w, gt_h), interpolation=cv2.INTER_NEAREST)  # 哦尺寸转换在这呢
