...
class Backproject(nn.Module):
    def __init__(self, batch_size, height, width):
        ...
        meshgrid = np.meshgrid(range(self.width), range(self.height), indexing='xy')   # 生成那两个网格矩阵，给每个像素权重坐标
        self.id_coords = np.stack(meshgrid, axis=0).astype(np.float32)   # 拼接成两个通道的
        self.id_coords = torch.from_numpy(self.id_coords)
        self.ones = torch.ones(self.batch_size, 1, self.height * self.width)
        self.pix_coords = torch.unsqueeze(torch.stack([self.id_coords[0].view(-1), self.id_coords[1].view(-1)], 0), 0)   # 两个矩阵分别转成一维，然后堆叠两个一维张量，产生2*(h*w)的张量，然后unsqueeze增加两维（1,2，h*w）
        self.pix_coords = self.pix_coords.repeat(batch_size, 1, 1)   #沿着第一维度重复batchsize次 （batchsize,2，h*w）
    ...
    def forward(self, depth, inv_K):
        ...
        cam_points = depth.view(cam_points.shape[0], 1, -1) * cam_points   # [batch_size, 1, height, width]-> [batch_size, 1, height × width]
# cam_points torch.Size([2, 3, 184320])
# depth.view(self.batch_size, 1, -1) torch.Size([1, 1, 368640])
        #import ipdb
        #ipdb.set_trace()
